...
SFixedArray<unsigned char, 212> g_ucMaxPassengers = {3, 1,   1,   1,   3,   3, 0,   1,  1, 3, 1, 1,   1, 3, 1, 1,            // 400->415
...
{
    return IsOriginalModel(ulModel) || IsExtraModel(ulModel);
}

bool CClientVehicleManager::IsOriginalModel(unsigned long ulModel)
{
    return ulModel >= 400 && ulModel <= 611;
}

bool CClientVehicleManager::IsExtraModel(unsigned long ulModel)
{
    return ulModel >= 11682 && ulModel <= 12799;
}
...
bool CClientVehicleManager::SetMaxPassengerCount(unsigned long ulModel, unsigned char ucMaxCount)
{
...
unsigned char CClientVehicleManager::GetMaxPassengerCount(unsigned long ulModel)
{
    ...
    if (IsExtraModel(ulModel))
    {
        return g_ucMaxExtraPassengers[ulModel - 11682];
    }
...
void CClientVehicleManager::GetRandomVariation(unsigned short usModel, unsigned char& ucVariant, unsigned char& ucVariant2)
{
    ...
    ucVariant2 = 255;
    // Valid model?
    if (IsOriginalModel(usModel) && g_ucVariants[usModel - 400] != 255)
    {
        // caddy || cropduster
...
{
    return (IsOriginalModel(ulModel) && (g_ulVehicleAttributes[ulModel - 400] & VEHICLE_HAS_TURRENT));
}

bool CClientVehicleManager::HasSirens(unsigned long ulModel)
{
    return (IsOriginalModel(ulModel) && (g_ulVehicleAttributes[ulModel - 400] & VEHICLE_HAS_SIRENS));
}

bool CClientVehicleManager::HasTaxiLight(unsigned long ulModel)
{
    return (IsOriginalModel(ulModel) && (g_ulVehicleAttributes[ulModel - 400] & VEHICLE_HAS_TAXI_LIGHTS));
}

bool CClientVehicleManager::HasSearchLight(unsigned long ulModel)
{
    return (IsOriginalModel(ulModel) && (g_ulVehicleAttributes[ulModel - 400] & VEHICLE_HAS_SEARCH_LIGHT));
}

bool CClientVehicleManager::HasLandingGears(unsigned long ulModel)
{
    return (IsOriginalModel(ulModel) && (g_ulVehicleAttributes[ulModel - 400] & VEHICLE_HAS_LANDING_GEARS));
}

bool CClientVehicleManager::HasAdjustableProperty(unsigned long ulModel)
{
    return (IsOriginalModel(ulModel) && (g_ulVehicleAttributes[ulModel - 400] & VEHICLE_HAS_ADJUSTABLE_PROPERTY));
}

bool CClientVehicleManager::HasSmokeTrail(unsigned long ulModel)
{
    return (IsOriginalModel(ulModel) && (g_ulVehicleAttributes[ulModel - 400] & VEHICLE_HAS_SMOKE_TRAIL));
}
