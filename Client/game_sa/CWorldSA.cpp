...
void DrawCollisionDebug(CPhysicalSAInterface* pObject)
{
    CModelInfoSA* pModelInfoSA = (CModelInfoSA*)(pGame->GetModelInfo(pObject->m_nModelIndex));
    if (!pModelInfoSA)
        return;
    ...
    {
        CMatrix objMat;
        if (pObject->Placeable.matrix)
            pObject->Placeable.matrix->ConvertToMatrix(objMat);
        else
            objMat.SetPosition(pObject->Placeable.m_transform.m_translate);
        ...
        for (int i = 0; i < pColData->numColTriangles; i++)
        {
            ...
            trianglePosition[0] = objMat.TransformVector(pColData->pVertices[pColTriangle->v1].getVector());
            trianglePosition[1] = objMat.TransformVector(pColData->pVertices[pColTriangle->v2].getVector());
            trianglePosition[2] = objMat.TransformVector(pColData->pVertices[pColTriangle->v3].getVector());
        ...
        for (int i = 0; i < pColData->numColBoxes; i++)
        {
            ...
            // vertical lines
            g_pCore->GetGraphics()->DrawLine3DQueued(objMat.TransformVector(CVector(posMin.fX, posMin.fY, posMin.fZ)),
                                                     objMat.TransformVector(CVector(posMin.fX, posMin.fY, posMax.fZ)), fLineWidth, color, false);
            g_pCore->GetGraphics()->DrawLine3DQueued(objMat.TransformVector(CVector(posMax.fX, posMin.fY, posMin.fZ)),
                                                     objMat.TransformVector(CVector(posMax.fX, posMin.fY, posMax.fZ)), fLineWidth, color, false);
            g_pCore->GetGraphics()->DrawLine3DQueued(objMat.TransformVector(CVector(posMax.fX, posMax.fY, posMin.fZ)),
                                                     objMat.TransformVector(CVector(posMax.fX, posMax.fY, posMax.fZ)), fLineWidth, color, false);
            g_pCore->GetGraphics()->DrawLine3DQueued(objMat.TransformVector(CVector(posMin.fX, posMax.fY, posMin.fZ)),
                                                     objMat.TransformVector(CVector(posMin.fX, posMax.fY, posMax.fZ)), fLineWidth, color, false);

            // one corner
            g_pCore->GetGraphics()->DrawLine3DQueued(objMat.TransformVector(CVector(posMin.fX, posMin.fY, posMin.fZ)),
                                                     objMat.TransformVector(CVector(posMin.fX, posMax.fY, posMin.fZ)), fLineWidth, color, false);
            g_pCore->GetGraphics()->DrawLine3DQueued(objMat.TransformVector(CVector(posMin.fX, posMin.fY, posMin.fZ)),
                                                     objMat.TransformVector(CVector(posMax.fX, posMin.fY, posMin.fZ)), fLineWidth, color, false);
            g_pCore->GetGraphics()->DrawLine3DQueued(objMat.TransformVector(CVector(posMin.fX, posMin.fY, posMax.fZ)),
                                                     objMat.TransformVector(CVector(posMin.fX, posMax.fY, posMax.fZ)), fLineWidth, color, false);
            g_pCore->GetGraphics()->DrawLine3DQueued(objMat.TransformVector(CVector(posMin.fX, posMin.fY, posMax.fZ)),
                                                     objMat.TransformVector(CVector(posMax.fX, posMin.fY, posMax.fZ)), fLineWidth, color, false);
            ...
            g_pCore->GetGraphics()->DrawLine3DQueued(objMat.TransformVector(CVector(posMax.fX, posMax.fY, posMin.fZ)),
                                                     objMat.TransformVector(CVector(posMax.fX, posMin.fY, posMin.fZ)), fLineWidth, color, false);
            g_pCore->GetGraphics()->DrawLine3DQueued(objMat.TransformVector(CVector(posMax.fX, posMax.fY, posMax.fZ)),
                                                     objMat.TransformVector(CVector(posMin.fX, posMax.fY, posMax.fZ)), fLineWidth, color, false);
            g_pCore->GetGraphics()->DrawLine3DQueued(objMat.TransformVector(CVector(posMax.fX, posMax.fY, posMax.fZ)),
                                                     objMat.TransformVector(CVector(posMax.fX, posMin.fY, posMax.fZ)), fLineWidth, color, false);
        }
        ...
        for (int i = 0; i < pColData->numColSpheres; i++)
        {
            ...
            spherePos = objMat.TransformVector(pColSphere->vecCenter);
            CGraphicsInterface* pGraphics = g_pCore->GetGraphics();

            uint iterations = Clamp(1, Round(pow(pColSphere->fRadius, 0.25f) * 0.75f), 4);

            const SWireModel& model = GetSphereWireModel(iterations);

            for (uint i = 0; i < model.vertexList.size(); i += 2)
            {
                const CVector& vecBegin = model.vertexList[i] * pColSphere->fRadius + spherePos;
                const CVector& vecEnd = model.vertexList[i + 1] * pColSphere->fRadius + spherePos;
...
void DrawColModel(CEntitySAInterface* pEntity)
{
    if (!g_pCore->GetGame()->IsCollisionDebugEnabled())
        return;

    if (pEntity->nType == ENTITY_TYPE_BUILDING || pEntity->nType == ENTITY_TYPE_OBJECT)
    {
        CPhysicalSAInterface* pPhysicalObject = static_cast<CPhysicalSAInterface*>(pEntity);
        if (pPhysicalObject)
        {
            if (!pPhysicalObject->bUsesCollision || !pPhysicalObject->bUsesCollision || pPhysicalObject->IsLowLodEntity())
                return;

            DrawCollisionDebug(pPhysicalObject);
        }
    }
}
...
{
    static CColModelSAInterface* colModel;
