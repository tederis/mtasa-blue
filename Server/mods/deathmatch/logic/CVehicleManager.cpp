...
{
    return IsOriginalModel(ulVehicleModel) || IsExtraModel(ulVehicleModel);
}

bool CVehicleManager::IsOriginalModel(unsigned int ulVehicleModel)
{
    return ulVehicleModel >= 400 && ulVehicleModel <= 611;
}

bool CVehicleManager::IsExtraModel(unsigned int ulVehicleModel)
{
    return ulVehicleModel >= 11682 && ulVehicleModel <= 12799;
}

eVehicleType CVehicleManager::GetVehicleType(unsigned short usModel)
{
    if (IsExtraModel(usModel))
        return VEHICLE_CAR;
...
{
    return (IsOriginalModel(uiModel) && (g_ulVehicleAttributes[uiModel - 400] & VEHICLE_HAS_TURRENT));
}

bool CVehicleManager::HasSirens(unsigned int uiModel)
{
    return (IsOriginalModel(uiModel) && (g_ulVehicleAttributes[uiModel - 400] & VEHICLE_HAS_SIRENS));
}

bool CVehicleManager::HasTaxiLight(unsigned int uiModel)
{
    return (IsOriginalModel(uiModel) && (g_ulVehicleAttributes[uiModel - 400] & VEHICLE_HAS_TAXI_LIGHTS));
}

bool CVehicleManager::HasLandingGears(unsigned int uiModel)
{
    return (IsOriginalModel(uiModel) && (g_ulVehicleAttributes[uiModel - 400] & VEHICLE_HAS_LANDING_GEARS));
}

bool CVehicleManager::HasAdjustableProperty(unsigned int uiModel)
{
    return (IsOriginalModel(uiModel) && (g_ulVehicleAttributes[uiModel - 400] & VEHICLE_HAS_ADJUSTABLE_PROPERTY));
}

bool CVehicleManager::HasSmokeTrail(unsigned int uiModel)
{
    return (IsOriginalModel(uiModel) && (g_ulVehicleAttributes[uiModel - 400] & VEHICLE_HAS_SMOKE_TRAIL));
}

bool CVehicleManager::IsTrailer(unsigned int uiVehicleModel)
{
    return (IsOriginalModel(uiVehicleModel) && (gs_vehicleTypes[uiVehicleModel - 400] == VEHICLE_TRAILER));
}
